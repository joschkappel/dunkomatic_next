...
class LeagueController extends Controller
{
    ...
    public function list($language, Region $region)
    {
        ...
        return $leaguelist
            ...
            ->editColumn('shortname', function ($l) {
                ...
                return ['display' => $link, 'sort' => $l->shortname];
            })
            ->editColumn('age_type', function ($l) {
                return ['display' => LeagueAgeType::getDescription($l->age_type), 'sort' => $l->age_type];
            })
            ->editColumn('gender_type', function ($l) {
                return ['display' => LeagueGenderType::getDescription($l->gender_type), 'sort' => $l->gender_type];
            })
            ->addColumn('size', function ($l) {
                if ($l->schedule != null) {
                    return ($l->size == null) ? ['display' => null, 'sort' => 0] : ['display' => $l->size, 'sort' => $l->size];
                } else {
                    return ['display' => null, 'sort' => 0];
                }
            })
    ...
    public function list_mgmt(Request $request, $language, Region $region)
    {
        ...
        foreach ($leagues as $l) {
            [$clubteam, $c_keys, $t_keys] = $this->get_registrations($l);
            ...
            for ($i = 1; $i < 17; $i++) {
                if ($i <= $l->size) {
                    ...
                    [$btn_status, $btn_color, $btn_text, $btn_function, $subbtn_color] = $this->get_button_settings(
                        $l,
                        Auth::user(),
                        $c['club_id'],
                        $c['team_id'],
    ...
    {
        [$clubteam, $c_keys, $t_keys] = $this->get_registrations($league);
        $available_no = $t_keys->collect();
        ...
        return $teamlist
            ...
            ->editColumn('club_shortname', function ($ct) use ($league, &$c_keys, $regions) {
                ...
                return ['display' => $btn, 'sort' => $sortkey];
            })
