...
class CustomLeagueGameImport implements ToCollection, WithStartRow, WithValidation, WithCustomCsvSettings
{
    ...
    public function prepareForValidation(array $data): array
    {
        ...
        $team_home = Team::where('club_id', $data['club_home_id'])->where('team_no', Str::substr($data[4], 4, 1))->where('league_id', $league->id)->first();
        ...
        $team_guest = Team::where('club_id', $data['club_guest_id'])->where('team_no', Str::substr($data[5], 4, 1))->where('league_id', $league->id)->first();
