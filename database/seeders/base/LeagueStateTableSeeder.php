    ...
    public function run()
    {
        ...
        foreach ($leagues as $l) {
            if (($l->schedule()->exists()) and ($l->league_size()->exists())) {
                ...
                if ($kpis['charspicked'] > 0) {
                    $this->open_char_selection($l);
                    $l->refresh();
                }
                if ($kpis['generated'] > 0) {
                    $this->open_game_scheduling($l);
                    $l->refresh();
                }
                if (($kpis['generated'] > 0) and ($kpis['scheduled'] == $kpis['generated'])) {
                    $this->open_ref_assignment($l);
                    $l->refresh();
                }
                if (($kpis['generated'] > 0) and ($kpis['referees'] == $kpis['generated'])) {
