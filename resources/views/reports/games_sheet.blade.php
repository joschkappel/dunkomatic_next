   ...
   @foreach($games as $game)
        ...
        <tr>
           ...
           @if ($game->game_time != null)
             <td style="border: 1px solid #ffffff;padding: 3px 2px;{{$rstyle}}">{{ Carbon\Carbon::parse($game->game_time)->isoFormat('LT')}}</td>
           @else
             <td style="border: 1px solid #ffffff;padding: 3px 2px;"></td>
           @endif
           <td style="border: 1px solid #ffffff;padding: 3px 2px;{{$rstyle}}">{{ $game->league->shortname }}</td>
           <td style="text-align: right;border: 1px solid #FFFFFF;padding: 3px 2px;{{$rstyle}}">{{ $game->game_no }}</td>
           <td style="border: 1px solid #FFFFFF;padding: 3px 2px;{{$rstyle}}">{{ $game->team_home}}</td>
           <td style="border: 1px solid #FFFFFF;padding: 3px 2px;{{$rstyle}}">{{ $game->team_guest}}</td>
           <td style="border: 1px solid #FFFFFF;padding: 3px 2px;{{$rstyle}}">{{ $game->gym_no}}</td>
           <td style="border: 1px solid #FFFFFF;padding: 3px 2px;{{$rstyle}}">{{ ($game->referee_1 == '' or $game->referee_1 == '****') ? $game->referee_1 : $game->referee_1." / ".$game->referee_2}}</td>
        </tr>
   @endforeach
   </tbody>
</table>
