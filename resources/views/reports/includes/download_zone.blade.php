    ...
    <div class="card-deck">
        @if ($scope=='club')
            <div class="card border-dark mb-3 text-center">
                ...
                <div class="card-body">
                    ...
                    <a type="button" class="btn mb-1  btn-sm btn-block  btn-primary" href="{{ route('club_archive.get', ['club' => $entity, 'format'=>App\Enums\ReportFileType::XLSX]) }}">
                    {{Str::upper( App\Enums\ReportFileType::XLSX()->description ) }}<i class="fas fa-file-download fa-lg mx-2"></i>
                    </a>
                    @endif
                    @if ($entity->filecount(App\Enums\ReportFileType::HTML()) > 0)
                    <a type="button" class="btn mb-1  btn-sm btn-block  btn-secondary" href="{{ route('club_archive.get', ['club' => $entity, 'format'=>App\Enums\ReportFileType::HTML]) }}">
        ...
        @if ($scope=='league')
            <div class="card border-dark mb-3 text-center">
                ...
                <div class="card-body">
                    @if ($entity->filecount(App\Enums\ReportFileType::XLSX()) > 0)
                    <a type="button" class="btn mb-1  btn-sm btn-block  btn-primary" href="{{ route('league_archive.get', ['league' => $entity, 'format'=>App\Enums\ReportFileType::XLSX]) }}">
                    {{Str::upper( App\Enums\ReportFileType::XLSX()->description ) }}<i class="fas fa-file-download fa-lg mx-2"></i>
                    </a>
                    @endif
                    @if ($entity->filecount(App\Enums\ReportFileType::HTML()) > 0)
                    <a type="button" class="btn mb-1  btn-sm btn-block  btn-secondary" href="{{ route('league_archive.get', ['league' => $entity, 'format'=>App\Enums\ReportFileType::HTML]) }}">
                    {{Str::upper( App\Enums\ReportFileType::HTML()->description ) }}<i class="fas fa-file-download fa-lg mx-2"></i>
                    </a>
                    @endif
        ...
        <div class="card border-dark mb-3 text-center">
            ...
            <div class="card-body">
                ...
                <a type="button" class="btn mb-1  btn-sm btn-block  btn-primary" href="{{ route('region_members_archive.get', ['region' => $region, 'format'=>App\Enums\ReportFileType::XLSX]) }}">
                {{Str::upper( App\Enums\ReportFileType::XLSX()->description ) }}<i class="fas fa-file-download fa-lg mx-2"></i>
                </a>
                @endif
                @if ($region->filecount(App\Enums\ReportFileType::HTML(),  App\Enums\Report::AddressBook()->getReportFilename()) > 0)
                <a type="button" class="btn mb-1  btn-sm btn-block  btn-secondary" href="{{ route('region_members_archive.get', ['region' => $region, 'format'=>App\Enums\ReportFileType::HTML]) }}">
                {{Str::upper( App\Enums\ReportFileType::HTML()->description ) }}<i class="fas fa-file-download fa-lg mx-2"></i>
                </a>
                @endif
                @foreach ( $region->fmt_league_reports->getFlags() as $format )
        ...
        <div class="card border-dark mb-3 text-center">
            ...
            <div class="card-body">
                    ...
                    <a type="button" class="btn mb-1  btn-sm btn-block btn-sm btn-block btn-primary" href="{{ route('region_archive.get', ['region' => $region, 'format'=>App\Enums\ReportFileType::XLSX]) }}">
                    {{ Str::upper(App\Enums\ReportFileType::XLSX()->description ) }}<i class="fas fa-file-download fa-lg mx-2"></i>
                    </a>
                    @endif
                    @if ($region->filecount(App\Enums\ReportFileType::HTML(), App\Enums\Report::RegionGames()->getReportFilename()) > 0)
                    <a type="button" class="btn mb-1  btn-sm btn-block  btn-secondary" href="{{ route('region_archive.get', ['region' => $region, 'format'=>App\Enums\ReportFileType::HTML]) }}">
                    {{Str::upper( App\Enums\ReportFileType::HTML()->description ) }}<i class="fas fa-file-download fa-lg mx-2"></i>
                    </a>
                    @endif
                    @foreach ( $region->fmt_league_reports->getFlags() as $format )
        ...
        <div class="card border-dark mb-3 text-center">
            ...
            <div class="card-body">
                ...
                <a type="button" class="btn mb-1  btn-sm btn-block  btn-primary" href="{{ route('region_league_archive.get', ['region' => $region, 'format'=>App\Enums\ReportFileType::XLSX]) }}">
                {{Str::upper( App\Enums\ReportFileType::XLSX()->description ) }}<i class="fas fa-file-download fa-lg mx-2"></i>
                </a>
                @endif
                ...
                <a type="button" class="btn mb-1  btn-sm btn-block  btn-secondary" href="{{ route('region_league_archive.get', ['region' => $region, 'format'=>App\Enums\ReportFileType::HTML]) }}">
                {{Str::upper( App\Enums\ReportFileType::HTML()->description ) }}<i class="fas fa-file-download fa-lg mx-2"></i>
                </a>
                @endif
                @foreach ( $region->fmt_league_reports->getFlags() as $format )
        ...
        @if ($scope=='region')
            <div class="card border-dark mb-3 text-center">
                ...
                <div class="card-body">
                    ...
                    <a type="button" class="btn mb-1  btn-sm btn-block  btn-primary" href="{{ route('region_teamware_archive.get', ['region' => $region]) }}">
                        {{Str::upper( App\Enums\ReportFileType::CSV()->description ) }}<i class="fas fa-file-download fa-lg mx-2"></i>
                    </a>
                    @endif
                </div>
