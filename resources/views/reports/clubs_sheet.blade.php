   ...
   @foreach($clubs as $c)
       ...
       @foreach($c['teams'] as $t)
         <tr>
           ...
           <td style="font-size: 12px;">{{ $t->load('members')->members->pluck('name')->implode(', ') }}</td>
           <td style="font-size: 12px;">@lang('team.shirtcolor'): {{ $t->shirt_color }}</td>
         </tr>
         <tr>
           <td></td>
           <td></td>
           <td style="font-size: 12px;">{{ $t->load('members')->members->pluck('email')->implode(', ') }}</td>
           <td style="font-size: 12px;">{{$t->load('members')->members->pluck('mobile')->implode(', ') }}</td>
         </tr>
        @endforeach
        @foreach($c['gyms'] as $g)
          <tr>
